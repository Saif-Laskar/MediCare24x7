...
def add_ambulance_view(
    request,
):  # add-ambulance view, this view is for staff to add ambulance
    if request.method == "POST":  # if the request is a post request
        form = AmbulanceForm(
            request.POST
        )  # create a form object with the data from the request
        if form.is_valid():  # if the form is valid
            form.save()  # save the form, if valid
            messages.success(
                request, "New Ambulance successfully Added!"
            )  # display success message, if the ambulance is successfully added
            return redirect(
                "staff-dashboard"
            )  # redirect to staff-dashboard view, after a successfull ambulance addition
    ...
    return render(
        request, "ambulance/add_ambulance.html", context
    )  # render the add_ambulance.html template with the context


@login_required(login_url="login")
def edit_ambulance_view(
    request, pk
):  # edit-ambulance view, this view is for staff to edit ambulance
...
def staff_abmulance_view(
    ...
    return render(
        request, "ambulance/staff-all-ambulance.html", context
...
def ambulance_detail_view(request, pk):
    ...
    if ambulance.city:
        has_city = True
        location_link = (
            "https://maps.google.com/maps?width=100%25&amp;height=450&amp;hl=en&amp;q="
            + ambulance.city
...
@login_required(login_url="login")
def patient_available_abmulance_view(
    request,
):  # staff-all-ambulance view, this view is for staff to view all ambulances
    ...
    return render(
        request, "ambulance/patient-available-abmulance.html", context
    )  # render the staff-all-ambulance.html template with the context
